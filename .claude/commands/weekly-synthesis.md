...
2. **Gather the week's data:**
   - Read daily reviews from `/mnt/nas/Files/06 Archive/Daily Reviews/` for dates from week start to current date
   - Read session summaries from `/mnt/nas/Files/06 Archive/Claude Sessions/` for the same date range
...
4. **Ensure directory exists:**
   - Check if `/mnt/nas/Files/06 Archive/Weekly Reviews/` directory exists
   - If not, create it: `mkdir -p "/mnt/nas/Files/06 Archive/Weekly Reviews"`
   - This prevents first-run failures

5. **Generate weekly synthesis:**

Create a file at `/mnt/nas/Files/06 Archive/Weekly Reviews/YYYY-Wnn.md` (using ISO week number from step 1):
