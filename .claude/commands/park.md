...
0. **Verify NAS mount accessibility** before proceeding:
   - Check if `/mnt/nas/Files` is accessible: `mountpoint -q /mnt/nas/Files`
...
5. **Determine session metadata:**
   - Session number for today (check existing file at `/mnt/nas/Files/06 Archive/Claude Sessions/YYYY-MM-DD.md` to find last session number, or start at 1)
   - Topic/name for this session (concise, descriptive)
   - Use current time from step 1 (already checked)
   - Related project (if applicable, from `/mnt/nas/Files/03 Projects/`)
...
8. **Write the summary** (with file locking for concurrent safety):
   ...
   - Create lock file before writing: `/mnt/nas/Files/06 Archive/Claude Sessions/.YYYY-MM-DD.md.lock`
   ...
   - Append to `/mnt/nas/Files/06 Archive/Claude Sessions/YYYY-MM-DD.md` (using current date from step 1)
...
9. **Update Works in Progress** (conditional on tier):
   ...
   - Read `/mnt/nas/Files/01 Now/Works in Progress.md`
